...
def test_audit_catches_bad_ohlc() -> None:
    bars = [
        Bar(symbol="T", ts=i, interval_seconds=86_400, open=100, high=110, low=90, close=100 + i,
            volume=100, available_time=i)
        for i in range(50)
    ]
...
def test_audit_clean_data_passes() -> None:
    bars = [Bar(symbol="T", ts=i * 86_400_000, interval_seconds=86_400, open=100, high=101, low=99, close=100.5, volume=100, available_time=i * 86_400_000) for i in range(120)]
    audit = audit_bars(bars)
...
def test_audit_dataset_pit_violation() -> None:
    ...
    row = PointInTimeRow.model_construct(
        symbol="T", ts=100, available_time=50, ingestion_time=60,
...
def test_bar_stream_hides_unavailable_bar() -> None:
    bars = [
        Bar(symbol="T", ts=i, interval_seconds=1, open=100, high=101, low=99, close=100 + i, volume=10,
            available_time=i)
        for i in range(10)
    ]
    bars[5] = Bar(symbol="T", ts=5, interval_seconds=1, open=100, high=101, low=99, close=100, volume=10,
                  available_time=9)  # late-reported: not usable until bar 9
...
def test_bar_stream_future_access_raises() -> None:
    bars = [Bar(symbol="T", ts=i, interval_seconds=1, open=100, high=101, low=99, close=100 + i, volume=10, available_time=i) for i in range(10)]
    stream = BarStream(bars, "T")
    try:
...
def test_snapshot_bar_requires_available_time() -> None:
    ...
    try:
        Bar(
            symbol="T", ts=1, interval_seconds=1,
            open=1, high=1, low=1, close=1,
        )
    ...
    try:
        SnapshotBar(
            symbol="T", ts=1, interval_seconds=1, open=1, high=1, low=1, close=1,
