...
BENCHMARK_NAMES = ["buy_and_hold", "random", "sma_crossover", "momentum", "mean_reversion", "breakout", "vol_target"]
...
def main() -> int:
    ...
    for process in available_processes():
        ...
        for name in BENCHMARK_NAMES:
            ...
            strat = create_strategy(name, {"seed": 2} if name == "random" else {"fast": 10, "slow": 30} if name == "sma_crossover" else None)
            ...
            result = BacktestEngine(BacktestConfig(max_position_weight=max_w)).run(strat, {series.symbol: series})
