...
def probabilistic_sharpe_ratio(
    returns, benchmark_sr: float = 0.0, periods_per_year: int = 252
) -> dict:
    ...
    if std <= 0:
        return {
            "psr": 1.0 if benchmark_sr <= 0 else 0.0,
            "n": n,
            "sr_hat": 0.0,
            "skew": 0.0,
            "kurtosis": 3.0,
    ...
    psr = (
        _normal_cdf((sr_hat - sr_bench) * math.sqrt(n - 1) / psi)
        if psi > 0
        else (1.0 if sr_hat >= sr_bench else 0.0)
    )
    return {
        "psr": float(psr),
        "n": n,
